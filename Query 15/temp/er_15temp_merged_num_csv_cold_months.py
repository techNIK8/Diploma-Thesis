...
electricheater_electric = df['electricheater_electric'].mean()
electric_heater = df['electric_heater'].mean()
...
df['high_temperature'] = df['temperature_per_cold_months_mean'].apply(lambda x: 1 if x > df['temperature_per_cold_months_mean'].mean() else 0)
...
df.to_csv(r'C:\Users\User\Desktop\household_sensors\er_15temp.csv', index=False)
