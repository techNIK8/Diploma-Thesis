"""
Extract the values from electric-mains_electric-combined.csv for each homeid 
...
path_to_figures = r'C:\Users\User\Desktop\household_sensors\electric-mains_electric-combined_graphs'
...
for d_path in filtered_file_paths:    #load csv in df
    ...
    for j in df:
        ...
        Y = df.values
        fig, ax = plt.subplots()
        ax.plot_date(X, Y, markerfacecolor='black', markeredgecolor='yellow', markeredgewidth=0.1 , ms=2)
        fig.autofmt_xdate()
    ...
    plt.legend(['Apparent Electric Power Mains/Combined (W)'], loc = 'upper left', fontsize=6)
