...
for each homeid, save only the values for hot months,
...
path_to_csv = r'C:\Users\User\Desktop\household_sensors\final_mean_electric-mains_electric-combined_hot_months.csv'
...
df_2 = pd.DataFrame(columns=['homeid', 'electric_con_per_hot_months_mean'])    #mean of electric_mains_electric_combined for hot months per homeid
i = 0
...
for d_path in filtered_file_paths:    #load csv in df  
    ...
    df_mask = df.loc[mask]  
    df_temp = pd.DataFrame([df_mask['electric_mains_electric_combined'].mean()], columns=['electric_con_per_hot_months_mean'])
...
df_2['electric_con_per_hot_months_mean'] = df_2['electric_con_per_hot_months_mean'].round(decimals = 2)     #round value to two decimals
