...
for each homeid, save only the values for winter time,
...
path_to_csv = r'C:\Users\User\Desktop\household_sensors\final_mean_electric-mains_electric-combined_winter_time.csv'
...
df_2 = pd.DataFrame(columns=['homeid', 'electric_con_per_winter_time_mean'])    #mean of electric_mains_electric_combined for winter time per homeid
i = 0
...
for d_path in filtered_file_paths:    #load csv in df
    ...
    df_mask = df.loc[mask]
    df_temp = pd.DataFrame([df_mask['electric_mains_electric_combined'].mean()], columns=['electric_con_per_winter_time_mean'])
...
df_2['electric_con_per_winter_time_mean'] = df_2['electric_con_per_winter_time_mean'].round(decimals = 2)     #round value to two decimals
