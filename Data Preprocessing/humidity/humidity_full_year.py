...
path_to_csv = r'C:\Users\User\Desktop\household_sensors\final_mean_humidity_per_room_full_year.csv'
path_to_csv_2 = r'C:\Users\User\Desktop\household_sensors\final_mean_humidity_full_year.csv'
...
df_2 = pd.DataFrame(columns=['homeid', 'room', 'humidity_per_full_year_mean'])    #mean of room_humidity for a full year per homeid
df_3 = pd.DataFrame
i = 0
...
for d_path in filtered_file_paths:    #load csv in df    
    ...
    df_temp = pd.DataFrame([df_mask['Humidity'].mean()], columns=['humidity_per_full_year_mean'])
    df_temp['humidity_per_full_year_mean'] = df_temp['humidity_per_full_year_mean'].apply(lambda x: x/10)    #humidity value in the dataset is multiplied x10
...
df_3 = df_2.groupby(['homeid'])['humidity_per_full_year_mean'].mean().reset_index()
df_3['homeid'] = df_3['homeid'].astype('int')    #delete decimal part of homeid in df_3
df_3['humidity_per_full_year_mean'] = df_3['humidity_per_full_year_mean'].round(decimals = 2)     #round value to two decimals
df_3.to_csv(path_to_csv_2, index=False)

df_2['homeid'] = df_2['homeid'].astype('int')    #delete decimal part of homeid in df_2
df_2['humidity_per_full_year_mean'] = df_2['humidity_per_full_year_mean'].round(decimals = 2)     #round value to two decimals
