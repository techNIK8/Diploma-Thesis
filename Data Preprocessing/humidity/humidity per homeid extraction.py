...
path_to_csv = r'C:\Users\User\Desktop\household_sensors\final_mean_humidity_per_room.csv'
path_to_csv_2 = r'C:\Users\User\Desktop\household_sensors\final_mean_humidity.csv'
...
df_2 = pd.DataFrame(columns=['homeid', 'room', 'humidity_mean'])    #mean of room_humidity per homeid
df_3 = pd.DataFrame
i = 0
...
for d_path in filtered_file_paths:    #load csv in df    
    ...
    df = df[df.Humidity != 0]
    df_temp = pd.DataFrame([df['Humidity'].mean()], columns=['humidity_mean'])
    df_temp['humidity_mean'] = df_temp['humidity_mean'].apply(lambda x: x/10)    #humidity value in the dataset is multiplied x10   
...
df_3 = df_2.groupby(['homeid'])['humidity_mean'].mean().reset_index()
df_3['homeid'] = df_3['homeid'].astype('int')    #delete decimal part of homeid in df_3
df_3['humidity_mean'] = df_3['humidity_mean'].round(decimals = 2)     #round value to two decimals
df_3.to_csv(path_to_csv_2, index=False)

df_2['homeid'] = df_2['homeid'].astype('int')    #delete decimal part of homeid in df_2
df_2['humidity_mean'] = df_2['humidity_mean'].round(decimals = 2)     #round value to two decimals
