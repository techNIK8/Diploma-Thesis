"""
Extract the values from room_humidity.csv.gz for each roomid 
...
path_to_figures = r'C:\Users\User\Desktop\household_sensors\humidity_graphs'
...
for d_path in filtered_file_paths:    #load csv in df  
    df = pd.read_csv(d_path, compression='gzip', sep=',', quotechar='"', 
                                            ...
                                            names=["Date", "Humidity"])
    df = df[df.Humidity != 0]
    df['Humidity'] = df['Humidity'].apply(lambda x: x/10)    #humidity value in the dataset is multiplied x10
    ...
    for j in df:
        ...
        Y = df.values
        fig, ax = plt.subplots()
        ax.plot_date(X, Y, markerfacecolor='black', markeredgecolor='white', markeredgewidth=0.1 , ms=2)
        fig.autofmt_xdate()
    ...
    plt.legend(['Relative Humidity (%)'], loc = 'upper left', fontsize=6)
