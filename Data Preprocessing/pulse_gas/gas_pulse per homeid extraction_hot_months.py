...
save only the values for hot months, calculate the average and 
...
path_to_csv = r'C:\Users\User\Desktop\household_sensors\final_mean_pulse_gas_hot_months.csv'

home_ids = []    #list of all home ids
home_ids_int = []   #list of all home ids converted to int
df = pd.DataFrame()    #pulse_gas csv
df_2 = pd.DataFrame(columns=['homeid', 'gas_pulse_per_hot_months_mean'])    #mean of gas pulse values for hot months per homeid
i = 0
...
for d_path in filtered_file_paths:    #load csv in df 
    ...
    df_mask = df.loc[mask]
    df_temp = pd.DataFrame([df_mask['gas_pulse'].mean()], columns=['gas_pulse_per_hot_months_mean'])
...
df_2['gas_pulse_per_hot_months_mean'] = df_2['gas_pulse_per_hot_months_mean'].round(decimals = 2)     #round value to two decimals
