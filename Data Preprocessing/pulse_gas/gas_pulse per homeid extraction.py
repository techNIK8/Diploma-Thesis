...
save all values, calculate the average and 
...
path_to_csv = r'C:\Users\User\Desktop\household_sensors\final_home_id_mean_pulse_gas.csv'

home_ids = []    #list of all home ids
home_ids_int = []   #list of all home ids converted to int
df = pd.DataFrame()    #pulse_gas csv
df_2 = pd.DataFrame(columns=["homeid", "gas_pulse_mean"])    #mean of gas pulse values per homeid
i = 0
...
for d_path in filtered_file_paths:    #φόρτωση των csv στο df   
    ...
    df_temp = pd.DataFrame([df['gas_pulse'].mean()], columns=['gas_pulse_mean'])
...
df_2['gas_pulse_mean'] = df_2['gas_pulse_mean'].round(decimals = 2)     #round value to two decimals
