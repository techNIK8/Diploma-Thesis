...
path_to_csv = r'C:\Users\User\Desktop\household_sensors\final_mean_temperature_per_room_hot_months.csv'
path_to_csv_2 = r'C:\Users\User\Desktop\household_sensors\final_mean_temperature_hot_months.csv'
...
df_2 = pd.DataFrame(columns=['homeid', 'room', 'temperature_per_hot_months_mean'])    #mean of room_temperature for cold months per homeid
df_3 = pd.DataFrame
i = 0
...
for d_path in filtered_file_paths:    #load csv in df   
    ...
    df_temp = pd.DataFrame([df_mask['Temperature'].mean()], columns=['temperature_per_hot_months_mean'])
    df_temp['temperature_per_hot_months_mean'] = df_temp['temperature_per_hot_months_mean'].apply(lambda x: x/10)    #temperature value in the dataset is multiplied x10
...
df_3 = df_2.groupby(['homeid'])['temperature_per_hot_months_mean'].mean().reset_index()
df_3['homeid'] = df_3['homeid'].astype('int')    #delete decimal part of homeid in df_3
df_3['temperature_per_hot_months_mean'] = df_3['temperature_per_hot_months_mean'].round(decimals = 2)     #round value to two decimals
df_3.to_csv(path_to_csv_2, index=False)

df_2['homeid'] = df_2['homeid'].astype('int')    #delete decimal part of homeid in df_2
df_2['temperature_per_hot_months_mean'] = df_2['temperature_per_hot_months_mean'].round(decimals = 2)     #round value to two decimals
