"""
Extract the humidity values from room_light.csv.gz for each roomid, 
...
path_to_csv = r'C:\Users\User\Desktop\household_sensors\final_mean_light_per_room.csv'
path_to_csv_2 = r'C:\Users\User\Desktop\household_sensors\final_mean_light.csv'
...
df_2 = pd.DataFrame(columns=['homeid', 'room', 'light_mean'])    #mean of light values per homeid
df_3 = pd.DataFrame
i = 0
...
for d_path in filtered_file_paths:    #load csv in df     
    ...
    df_temp = pd.DataFrame([df['Light'].mean()], columns=['light_mean'])  
...
df_3 = df_2.groupby(['homeid'])['light_mean'].mean().reset_index()
df_3['homeid'] = df_3['homeid'].astype('int')    #delete decimal part of homeid in df_3
df_3['light_mean'] = df_3['light_mean'].round(decimals = 2)     #round value to two decimals
df_3.to_csv(path_to_csv_2, index=False)

df_2['homeid'] = df_2['homeid'].astype('int')    #delete decimal part of homeid in df_2
df_2['light_mean'] = df_2['light_mean'].round(decimals = 2)     #round value to two decimals
