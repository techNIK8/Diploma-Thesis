"""
Extract the values from room_light.csv.gz for each roomid 
...
path_to_figures = r'C:\Users\User\Desktop\household_sensors\light_graphs'
...
for d_path in filtered_file_paths:    #load csv in df   
    ...
    for j in df:
        ...
        Y = df.values
        fig, ax = plt.subplots()
        ax.plot_date(X, Y, markerfacecolor='black', markeredgecolor='red', markeredgewidth=0.1 , ms=2)
        fig.autofmt_xdate()
    ...
    plt.legend(['Light (cd)'], loc = 'upper left', fontsize=6)
