...
save only the values for summer time, calculate the average for each homeid and 
...
path_to_csv = r'C:\Users\User\Desktop\household_sensors\final_mean_light_per_room_summer_time.csv'
path_to_csv_2 = r'C:\Users\User\Desktop\household_sensors\final_mean_light_summer_time.csv'
...
df_2 = pd.DataFrame(columns=['homeid', 'room', 'light_per_summer_time_mean'])    #mean of light values for summer time per homeid
df_3 = pd.DataFrame
i = 0
...
for d_path in filtered_file_paths:    #load csv in df  
    ...
    mask = ((df.index >= '2017-3-26') & (df.index < '2017-10-29')) | ((df.index >= '2018-3-25') & (df.index < '2018-7-1'))  
    df_mask = df.loc[mask]
    df_temp = pd.DataFrame([df_mask['Light'].mean()], columns=['light_per_summer_time_mean'])  
...
df_3 = df_2.groupby(['homeid'])['light_per_summer_time_mean'].mean().reset_index()
df_3['homeid'] = df_3['homeid'].astype('int')    #delete decimal part of homeid in df_3
df_3['light_per_summer_time_mean'] = df_3['light_per_summer_time_mean'].round(decimals = 2)     #round value to two decimals
df_3.to_csv(path_to_csv_2, index=False)

df_2['homeid'] = df_2['homeid'].astype('int')    #delete decimal part of homeid in df_2
df_2['light_per_summer_time_mean'] = df_2['light_per_summer_time_mean'].round(decimals = 2)     #round value to two decimals
